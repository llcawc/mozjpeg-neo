...
// BinWrapper class for platform-specific binary management
// Класс BinWrapper для управления платформозависимыми бинарными файлами
 ...
 */
const binWrapper: BinWrapper = new BinWrapper()
  .src(join(vendorPath, 'macos', 'amd64', 'cjpeg'), 'darwin', 'x64')
  .src(join(vendorPath, 'macos', 'arm64', 'cjpeg'), 'darwin', 'arm64')
  .src(join(vendorPath, 'linux', 'amd64', 'cjpeg'), 'linux', 'x64')
  .src(join(vendorPath, 'linux', 'arm64', 'cjpeg'), 'linux', 'arm64')
  .src(join(vendorPath, 'win', 'x86', 'cjpeg.exe'), 'win32', 'x86')
  .src(join(vendorPath, 'win', 'x64', 'cjpeg.exe'), 'win32', 'x64')
  .dest(vendorPath)
 ...
 */
const mozjpeg: string = binWrapper.path()
 ...
 */
export { mozjpeg, binWrapper as bin }
