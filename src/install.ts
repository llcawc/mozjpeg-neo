...
async function makeExecutable(binaryPath: string): Promise<void> {
  if (process.platform === 'win32') {
    // On Windows, executable permissions are not required
    // На Windows права на выполнение не требуются
    return
  }
  try {
 ...
 */
export async function ensureBinaryExecutable(): Promise<void> {
  const binaryPath = bin.path()
  if (!fileExists(binaryPath)) {
    throw new Error(`Binary not found at ${binaryPath}. Pre-built binary may be missing for this platform.`)
  }

  // Check if the file is executable (Unix only)
  // Проверяем, является ли файл исполняемым (только для Unix)
  if (process.platform !== 'win32') {
    try {
